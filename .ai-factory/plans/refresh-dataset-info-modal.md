...
- [ ] **D7.** Кнопка «?» рядом с темой → модалка (датасет + методология) с a11y — *blocked by D6*.
- [ ] **D8.** Гейт приёмки: тесты + ruff + check_html + браузерная проверка модалки/диапазона; бюджет — *blocked by D6, D7*.
